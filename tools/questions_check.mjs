...
// the twenty question start (v17.3): the six ids named in the page exist, are core, weight 3, and cover every cell (default bank path only)
if(!process.argv[2]){ try{ const page=fs.readFileSync(new URL('../docs/index.html',import.meta.url),'utf8'); const m=/const PK_START_QS=\[([^\]]*)\]/.exec(page); if(!m) bad('PK_START_QS not found in the page'); else { const ids=m[1].split(',').map(x=>x.trim().replace(/^['"]|['"]$/g,'')).filter(Boolean); const byId=Object.fromEntries(rows.map(r=>[r&&r.id,r])); const cov=new Set(); ids.forEach(id=>{ const r=byId[id]; if(!r) bad('start question '+id+' is not in the bank'); else { if(r.facet!=='core') bad(id+' is not core'); if(r.weight!==3) bad(id+' is not weight 3'); cov.add(r.cell); } }); if(ids.length!==6) bad('PK_START_QS has '+ids.length+' ids (want 6)'); CELLS.forEach(c=>{ if(!cov.has(c)) bad('PK_START_QS misses cell '+c); }); } }catch(e){ bad('page unreadable: '+e.message); } }
