...
function __rpc(fn,a){ const g1=__H.grids[0]&&__H.grids[0].id; if(fn==='twingrid_spark_score'&&a&&a.p_grid===g1) return [{lane:'human',avg_rating:4.6,n:12},{lane:'persona',avg_rating:4.0,n:3}]; if(fn==='twingrid_kindred_of'&&a&&a.p_grid===g1) return __H.grids.slice(1).map(g=>({grid_id:g.id,name:g.name})); if(fn==='twingrid_visit_count') return 6; return []; }
